<div align="center">
  <img src="banner.jpg" alt="Juan Manuel Santos - Banner" width="100%">
  <br><br>
  🌍 <strong>Read this in:</strong> <a href="#english">English</a> | <a href="#español">Español</a>
</div>

<br>

<h1 id="english">🦊 Hi! I'm Manu Santos</h1>

**Mathematics Student | Programmer | Cybersecurity Enthusiast**

As a Mathematics student at the University of Málaga, I discovered that my true calling isn't just solving abstractions on paper, but using that pure logic to design efficient algorithms and protect network architectures. My professional goal is to be the bridge between mathematical rigor, software development, and corporate cybersecurity.

I dedicate myself to finding the exact point where theory collides with the real world. If a problem has a solution, I try to prove it and code it; and if someone tries to exploit it... I work to secure it.

### 🎯 My Current Focus
* 🛡️ Exploring the offensive and defensive sides of cybersecurity (Ethical Hacking, Cryptography).
* ⚛️ Tinkering with **Quantum Computing** and implementing circuits using Qiskit.
* 🧠 Training my mathematical brain to think like an attacker and defend like a cryptanalyst.

### 🛠️ Tech Stack
**Languages & Paradigms**

![Python](https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white)
![Scala](https://img.shields.io/badge/Scala-DC322F?style=for-the-badge&logo=scala&logoColor=white)
![Haskell](https://img.shields.io/badge/Haskell-5e5086?style=for-the-badge&logo=haskell&logoColor=white)

**Mathematics, Data & Environments**

![R](https://img.shields.io/badge/RStudio-276DC3?style=for-the-badge&logo=r&logoColor=white)
![LaTeX](https://img.shields.io/badge/LaTeX-008080?style=for-the-badge&logo=overleaf&logoColor=white)
![Mathematica](https://img.shields.io/badge/Mathematica-DD1100?style=for-the-badge&logo=wolframmathematica&logoColor=white)
![Jupyter](https://img.shields.io/badge/Jupyter-F37626?style=for-the-badge&logo=jupyter&logoColor=white)
![Anaconda](https://img.shields.io/badge/Anaconda-44A833?style=for-the-badge&logo=anaconda&logoColor=white)
![Stellarium](https://img.shields.io/badge/Stellarium-2A385C?style=for-the-badge&logoColor=white)

**Workflows & AI (The Art of Vibe Coding)**

![Prompt Engineering](https://img.shields.io/badge/Prompt_Engineering-FF9900?style=for-the-badge&logo=openai&logoColor=white)
![AI Assisted](https://img.shields.io/badge/AI_Assisted_Dev-000000?style=for-the-badge&logo=githubcopilot&logoColor=white)

> **🤖 My relationship with Web Development**
> If you look closely, there are no HTML, CSS, or JavaScript badges in my stack. It's not that I dislike them; I just prefer to use them as a conductor. I apply *vibe coding*: I focus on designing the logical architecture, solving mathematical puzzles, and thinking about attack vectors. Then, I rely on Artificial Intelligence to write the UI code for me. Let's just say I know exactly how to speak the language of machines so they can do the heavy lifting while I keep the fun part.

### 🌐 Where to find me
[![Portfolio](https://img.shields.io/badge/Portfolio_&_CV-05122A?style=for-the-badge&logo=github&logoColor=white)](https://manusantos-dev.github.io/JuanManuelSantos_CV/)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/juan-manuel-santos-dev/)

---
<br><br>

<h1 id="español">🦊 ¡Hola! Soy Manu Santos</h1>

**Estudiante de Matemáticas | Programador | Entusiasta de la Ciberseguridad**

Como estudiante del Grado en Matemáticas en la Universidad de Málaga, descubrí que mi verdadera vocación no está solo en resolver abstracciones en un papel, sino en utilizar esa lógica pura para diseñar algoritmos eficientes y proteger arquitecturas de red. Mi objetivo profesional es ser el puente entre el rigor matemático, el desarrollo de software y la ciberseguridad corporativa.

Me dedico a buscar el punto exacto donde la teoría choca con el mundo real. Si un problema tiene solución, intento demostrarla y programarla; y si alguien intenta vulnerarla... trabajo para asegurarla.

### 🎯 Mi Foco Actual
* 🛡️ Explorando el lado ofensivo y defensivo de la ciberseguridad (Hacking Ético, Criptografía).
* ⚛️ Trasteando con **Computación Cuántica** e implementando circuitos con Qiskit.
* 🧠 Entrenando mi cerebro matemático para pensar como un atacante y defender como un criptoanalista.

### 🛠️ Stack Tecnológico
**Lenguajes y Paradigmas**

![Python](https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white)
![Scala](https://img.shields.io/badge/Scala-DC322F?style=for-the-badge&logo=scala&logoColor=white)
![Haskell](https://img.shields.io/badge/Haskell-5e5086?style=for-the-badge&logo=haskell&logoColor=white)

**Matemáticas, Datos y Entornos**

![R](https://img.shields.io/badge/RStudio-276DC3?style=for-the-badge&logo=r&logoColor=white)
![LaTeX](https://img.shields.io/badge/LaTeX-008080?style=for-the-badge&logo=overleaf&logoColor=white)
![Mathematica](https://img.shields.io/badge/Mathematica-DD1100?style=for-the-badge&logo=wolframmathematica&logoColor=white)
![Jupyter](https://img.shields.io/badge/Jupyter-F37626?style=for-the-badge&logo=jupyter&logoColor=white)
![Anaconda](https://img.shields.io/badge/Anaconda-44A833?style=for-the-badge&logo=anaconda&logoColor=white)
![Stellarium](https://img.shields.io/badge/Stellarium-2A385C?style=for-the-badge&logoColor=white)

**Flujos de Trabajo & IA (El arte del Vibe Coding)**

![Prompt Engineering](https://img.shields.io/badge/Prompt_Engineering-FF9900?style=for-the-badge&logo=openai&logoColor=white)
![AI Assisted](https://img.shields.io/badge/AI_Assisted_Dev-000000?style=for-the-badge&logo=githubcopilot&logoColor=white)

> **🤖 Mi relación con el Desarrollo Web**
> Si te fijas, en mi *stack* no hay medallas de HTML, CSS o JavaScript. No es que les tenga manía, es que prefiero usarlos en modo director de orquesta. Aplico el *vibe coding*: me centro en diseñar la arquitectura lógica, resolver los rompecabezas matemáticos y pensar en los vectores de ataque. Luego, me apoyo en la Inteligencia Artificial para que pique el código de la interfaz por mí. Digamos que sé hablar exactamente el idioma de las máquinas para que ellas hagan el trabajo sucio mientras yo me quedo con la parte divertida.

### 🌐 Dónde encontrarme
[![Portfolio](https://img.shields.io/badge/Portfolio_&_CV-05122A?style=for-the-badge&logo=github&logoColor=white)](https://manusantos-dev.github.io/JuanManuelSantos_CV/)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/juan-manuel-santos-dev/)

---
> *"En matemáticas no entiendes las cosas. Solo te acostumbras a ellas."* - John von Neumann
